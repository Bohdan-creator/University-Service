...
namespace SchoolRegisterSystem.Areas.Identity.Pages.Account.Manage
{
    ...
    public class RegisterNewUsersDataModel : PageModel
    {
        ...
        public void OnGet()
        {
            ViewData["Roles"] = new SelectList(_dbContext.Roles.Select(t => new
            {
                Text = t.Name,
                Value = t.Id
            }), "Value", "Text", _dbContext.Roles.FirstOrDefault(x => x.RoleValue ==
           RoleValue.Parent).Id);
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var result = await _userManager.CreateAsync(tupleUserRole.Item1,
               Input.Password);
                if (result.Succeeded)
                {
                    _logger.LogInformation("User created a new account with password.");
                    var code = await
                   _userManager.GenerateEmailConfirmationTokenAsync(tupleUserRole.Item1);
                   ...
                   values: new { userId = tupleUserRole.Item1.Id, code = code },
                    ...
                    message.To.Add(tupleUserRole.Item1.Email);
                    message.From = new MailAddress("kucherbogdan2000@gmail.com");
                    message.Subject = "Account Service";
                    message.Body = "You have create account like"+ tupleUserRole.Item1.SecondName;
                    smpt.Send(message);

                    await _userManager.AddToRoleAsync(tupleUserRole.Item1,
            ...
            switch (role.RoleValue)
            {
                case RoleValue.Student:
                    return new Tuple<User, Role>(new Student
                    {
                        ...
                        SecondName = inputModel.LastName,
        ...
        public class InputModel
        {
            ...
            [Compare("Password", ErrorMessage = "The password and confirmation password do notmatch.")]
